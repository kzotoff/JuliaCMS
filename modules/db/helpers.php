...
class J_DB_Helpers {
	...
	/**
	 * Returns full field description compiled from original description (Registry -> 'api_fields') and
	...
	public static function getFullFieldDefinition($field_part1, $field_part2 = array()) {
		...
		if (is_numeric($field_part1) && is_string($field_part2)) {
			...
			if (isset($R['api_fields'][$field_part2])) {
				return $R['api_fields'][$field_part2];
			}
		...
		if (is_string($field_part1) && is_array($field_part2)) {
			$R = Registry::GetInstance();
			if (isset($R['api_fields'][$field_part1])) {
				return array_merge($R['api_fields'][$field_part1], $field_part2);
			} else {
	...
	public static function getFullActionDefinition($action_part1, $action_part2) {
		...
		$actions = array_merge(J_DB::$actions, UserLogic::$actions);
