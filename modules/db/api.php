...
class J_DB_API {
	...
	public static function generateTableXML($input, &$return_metadata, $DB) {
		...
		while ($data = $query->fetch(PDO::FETCH_ASSOC)) {
			...
			$data_row->setAttribute('id', $data[ CMS::$R['db_api_fields'][$report_config['id_field']]['field'] ]);
	...
	public static function generateEditorialXML($input, &$return_metadata, $DB) {
		...
		// generate new ID when creating record as we need to know it for adding comments
		if ($new_record) {
			$input['row_id'] = create_guid();
		}
