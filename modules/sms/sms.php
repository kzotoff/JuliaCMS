...
class J_SMS_ru extends JuliaCMSModule {
	...
	/**
	 * while creating sender, only API ID is required
	 */
	function __construct($module_name) {
		parent::__construct($module_name);
		$this->api_id = $this->CONFIG['api_id'];
	}
	 ...
	 */
	private function sendSingle($phone, $text, &$text_result, $options = array()) {
		...
		$url = str_replace(
			...
			array($this->api_id, $phone_filtered, urlencode($text), get_array_value($this->CONFIG, 'from', '')),
			$this->template_send
		);
...
//		$result = "100\n201531-1000005\nbalance=0";
	...
	private function sendFromJ_DB($row_id) {
		...
		if (!isset($modules_cache['db'])) { $modules_cache['db'] = new J_DB('db'); }
		$DB = $modules_cache['db']->DB;
	...
	/**
	 * SMS.ru notofication service responder
	 *
	 */
	...
	public function AJAXHandler() {
		...
		$filtered_input = get_filtered_input($input_filter);
		...
		switch ($filtered_input['action']) {
			...
			case 'send':
				return $this->sendFromJ_DB($filtered_input['row_id']);
				break;
