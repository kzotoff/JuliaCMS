...
class J_News extends JuliaCMSModule {
	...
	function contentGenerator($template) {
		...
		while (preg_match($placeholder, $template, $match) > 0) {
			...
			$str = $this->newsGenerate($news_template, get_array_value($menu_params, 'stream', 'default'), get_array_value($menu_params, 'count', -1));
	...
	function AJAXHandler() {
		...
		switch ($R['action']) {
			...
			case 'edit_elem':
				...
				if (!isset($modules_cache['menu'])) {
					$modules_cache['menu'] = new J_Menu('menu');
				}
				...
				$response = sprintf(MODULE_NEWS_MANAGE_ITEM_EDIT,
					...
					$row['summary'],
					$row['link'],
					$modules_cache['menu']->getPagesListAsSelect($row['page'], 'page', $DB),
					$row['streams']
				);
				break;
		}
