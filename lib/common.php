...
CMS::$R['USERFILES_DIRS'] = array(
	...
	'trash' => array( // must be the last
		'caption'         => 'trash',
		'dir'             => 'userfiles/trash/',
		'regexp_filename' => '[a-zA-Zа-яА-Я0-9]+[a-zA-Zа-яА-Я0-9\-_.]*\.[a-zA-Zа-яА-Я0-9]+',
		)
);
...
function module_init($module_name) {
	...
	if (!isset(CMS::$cache[$module_name]['config'])) {
		if (($config_result = module_get_config($module_name, $config)) !== true) {
			popup_message_add($config_result, JCMS_MESSAGE_ERROR);
			return false;
		}
		CMS::$cache[$module_name]['config'] = $config;
	}

	// third, init the module
...
function macro_regexp($type) {
	return '~(<macro\s[^>]*?type="'.$type.'"[^>]*(/>|(?<!/)>.*?</macro>)|\[macro\s[^\]]*?type="'.$type.'"[^\]]*(/\]|(?<!/)\].*?\[/macro\]))~';
}
...
function array_to_node($node, $array, $tags) {
	...
	foreach ($array as $index=>$value) {
		...
		$new_node = $node->ownerDocument->createElement($index);
 ...
 *                    root, second as first level sub-nodes, and so on. or just a string
 */
...
function get_filtered_input($filter, $options = array(FILTER_GET_BY_LIST, FILTER_POST_BY_LIST)) {
	...
	if (count($test_intersect = array_intersect_key($_POST, $_GET)) > 0) {
		logthis('get_filtered_input: GET and POST have duplicate keys: '.print_r(array_keys($test_intersect), 1), ZLogger::LOG_LEVEL_WARNING);
	}
	*/
