...
Log your messages with the code like this:
	...
	$logger::log('your message', ZLogger::LOG_LEVEL_MESSAGE);
...
Any time you need, make a dump with minimal level to show and some options (nothing at the moment)
	$logger::flushNow(ZLogger::LOG_LEVEL_MESSAGE, array('option_name'=>'value));
	or, for "return" output option,
	$result = $logger::flushNow(ZLogger::LOG_LEVEL_MESSAGE, array('option_name'=>'value'));
	or
	$logger::flushNow(ZLogger::LOG_LEVEL_MESSAGE);
	or even
	$logger::flushNow();
...
At any time, the entire log can be output with
	...
	$logger->flushAll(ZLogger::LOG_LEVEL_MESSAGE, array('option_name'=>'value'));
...
class ZLogger {
	 ...
	 * Log levels
	 *
	 * @const LOG_LEVEL_CRAZY
	 * @const LOG_LEVEL_DEBUG
	 * @const LOG_LEVEL_NOTICE
	 * @const LOG_LEVEL_MESSAGE
	 * @const LOG_LEVEL_WARNING
	 * @const LOG_LEVEL_ERROR
	 * @const LOG_LEVEL_FATAL
	 * @const LOG_LEVEL_CRASH
	 * @const LOG_LEVEL_ALARM
	 */
	const LOG_LEVEL_CRAZY   = 10; // ultra-deep mining
	const LOG_LEVEL_DEBUG   = 20; // standard debug-level catch
	const LOG_LEVEL_NOTICE  = 30; // not valuable messages (but indicates something unusual)
	const LOG_LEVEL_MESSAGE = 40; // standard message
	const LOG_LEVEL_WARNING = 50; // bad configs, missing parameters and so on
	const LOG_LEVEL_ERROR   = 60; // something that prevents some actions
	const LOG_LEVEL_FATAL   = 70; // so bad that we shall to terminate script
	const LOG_LEVEL_CRASH   = 80; // something even more bad
	const LOG_LEVEL_ALARM   = 90; // oops...
	...
	public static function singleton($options = array()) {
		...
		if (empty(self::$instance)) {
			...
			self::log('***************************************************************', self::LOG_LEVEL_MESSAGE);
			self::log(self::$locale['logger_started'], self::LOG_LEVEL_MESSAGE);

			// dump options in debug mode
			foreach (self::$options as $option_name => $value) {
				self::log('option set: '.$option_name.' = '.$value, self::LOG_LEVEL_DEBUG);
			}
...
/*
		if (count(self::$log) > 100) {
			self::flushNow(LOG_LEVEL_MESSAGE, array('target'=>'stdout'));
...
*/
	 ...
	 * @return void|false
	 */
	public static function flushNow($min_level = LOG_LEVEL_MESSAGE, $override_options = array()) {
	...
	public static function resetCounters($reset = array('time', 'memory')) {
		...
		if (in_array('time', $reset)) {
			...
			self::$instance->log(self::$instance->locale['reset_time'], self::LOG_LEVEL_NOTICE);
		}
		if (in_array('memory', $reset)) {
			self::$instance->memory_start = memory_get_usage();
			self::$instance->log(self::$instance->locale['reset_memory'], self::LOG_LEVEL_NOTICE);
		}
	}
	...
	private function logRecordToString($record_id, $min_level) {
		...
		switch ($log_record['level']) {
			case self::LOG_LEVEL_DEBUG:   $text_result = str_replace('%level%', self::$locale['level_debug'],   $text_result); break;
			case self::LOG_LEVEL_NOTICE:  $text_result = str_replace('%level%', self::$locale['level_notice'],  $text_result); break;
			case self::LOG_LEVEL_MESSAGE: $text_result = str_replace('%level%', self::$locale['level_message'], $text_result); break;
			case self::LOG_LEVEL_WARNING: $text_result = str_replace('%level%', self::$locale['level_warning'], $text_result); break;
			case self::LOG_LEVEL_ERROR:   $text_result = str_replace('%level%', self::$locale['level_error'],   $text_result); break;
			case self::LOG_LEVEL_FATAL:   $text_result = str_replace('%level%', self::$locale['level_fatal'],   $text_result); break;
			case self::LOG_LEVEL_CRASH:   $text_result = str_replace('%level%', self::$locale['level_crash'],   $text_result); break;
			case self::LOG_LEVEL_ALARM:   $text_result = str_replace('%level%', self::$locale['level_alarm'],   $text_result); break;
	...
	private function setLocale($set_locale = 'en') {
		...
		switch (strtoupper($set_locale)) {
			...
			default:
				self::$log('no localization found for "'.$set_locale.'"', LOG_LEVEL_WARNING);
				break;
		}
	}
